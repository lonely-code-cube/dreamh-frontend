<script lang="ts">
	...
	async function get_page(page: number) {
		let res = await fetch(`${PUBLIC_BACKEND_URL}/top-tiers?page=${page}&limit=15`);
		if (res.ok) {
			data.entries = await res.json();
		}
		window.scrollTo(0, 0);
		window.history.pushState(null, "", `/top-tiers?page=${currentPage}`);
	}
</script>
...
<div class="dark:text-white mt-10">
	<div class="w-[90%] m-auto bg-red-200 p-1 rounded-lg">
		<div class="bg-light-fg dark:bg-dark-fg p-5 rounded">
			<p class="text-2xl font-bold">Top Tiers</p>
			<p>Checkout our top tier entries!</p>
		</div>
	</div>
