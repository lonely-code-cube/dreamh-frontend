<script lang="ts">
	...
	async function get_page(page: number) {
		let res = await fetch(`${PUBLIC_BACKEND_URL}/top-rated?page=${page}&limit=15`);
		if (res.ok) {
			data.entries = await res.json();
		}
		window.scrollTo(0, 0);
		window.history.pushState(null, "", `/top-rated?page=${currentPage}`);
	}
</script>
...
<div class="dark:text-white mt-10">
	<div class="w-[90%] m-auto bg-red-200 p-1 rounded-lg">
		<div class="bg-light-fg dark:bg-dark-fg p-5 rounded">
			<p class="text-2xl font-bold">Top Rated</p>
			<p>Checkout our top rated entries!!</p>
		</div>
	</div>
